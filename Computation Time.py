...
Mandatory=[[1,3],[7,8],[11,13]]
Optional=[[4,4],[5,6],[9,10],[14,15]]

#标记产品和服务模块
Product=[1,2,3,4,5,6,11,12,13]
Service=[7,8,9,10,14,15]
...
Initial_M_Study=True
Initial_N_Study=False
Test_SampleSize_Study=False
k_Study=False

# FinalResults=pd.DataFrame(columns=['Initial_M_Study', 'Initial_N_Study', 'Test_SampleSize_Study','k_Study','Initial_M','Initial_N','Test_SampleSize','k','RandomSeed','Variance','time'])


def running(Randomseed,Variance,Initial_M,Initial_N,Test_SampleSize,k):
    np.random.seed(Randomseed)

    def ParaGenerator_Stochastic():
        Utility_tmp = Utility_Center + np.random.uniform(-Width_Utility, Width_Utility,
                                                         size=(Num_Demand, Num_Module))
        for d in range(Num_Demand):
            for i in Product:
        ...
        for d in range(Num_Demand):
            ...
            PairUtility_tmp[d][PairUtility_tmp[d] != 0] = PairUtility_tmp[d][
                                                              PairUtility_tmp[d] != 0] + np.random.uniform(
                -Width_Utility, Width_Utility, len(PairUtility_tmp[d][PairUtility_tmp[d] != 0]))
    ...
    def Optimizer(NN, Is_Stochastic):
        ...
        for sample in range(NN):
            ...
            obj1 = obj1 + quicksum(
                Z[i, p] * Wd[d] * Utility[d][i] * np.exp(-Sensitivity * Alternative_Price[i][p]) for d in
                range(Num_Demand) for i in range(Num_Module) for p in range(Num_Price)) \
                   + quicksum(
                Y[i, j] * Wd[d] * PairUtility[d][i][j] / 2 for d in range(Num_Demand) for i in range(Num_Module) for
                j in range(Num_Module))

            for d in range(Num_Demand):
                m.addConstr(
                    Storage[sample][d] == quicksum(X[i] * Utility[d][i] for i in range(Num_Module)) + quicksum(
                        Y[i, j] * PairUtility[d][i][j] / 2 for i in range(Num_Module) for j in range(Num_Module)))
    ...
    def Evaluation(DVX, DVY, DVZ, DVs, SampleSize, Utilitytmp, PairUtilitytmp, Sensitivitytmp):
        ...
        for TestID in range(SampleSize):
            Objectivetmp = Objectivetmp + sum(DVZ[i][p] * Wd[d] * Utilitytmp[TestID][d][i] * np.exp(
                -Sensitivitytmp[TestID] * Alternative_Price[i][p]) for d in range(Num_Demand) for i in
                                              range(Num_Module) for p in range(Num_Price)) \
                           + sum(
                DVY[i][j] * Wd[d] * PairUtilitytmp[TestID][d][i][j] / 2 for d in range(Num_Demand) for i in
                range(Num_Module) for j in range(Num_Module))
        ...
        for TestID in range(SampleSize):
            ...
            for d in range(Num_Demand):
                tmptmp.append(sum(DVX[i] * Utilitytmp[TestID][d][i] for i in range(Num_Module)) \
                              + sum(
                    DVY[i][j] * PairUtilitytmp[TestID][d][i][j] / 2 for i in range(Num_Module) for j in
                    range(Num_Module)))
    ...
    def SAA(M, N, N_test):
        ...
        while ((M < Final_M) and (N < Final_N)):
            ...
            while ((trials < M) and (Iterations < Max_Iterations)):
                [DecisionVarX[trials], DecisionVarY[trials], DecisionVarZ[trials], DecisionVars[trials],
                 Objective1[trials], Objective2[trials], Flag_of_Infeasible] = Optimizer(N, 'Stochastic')
            ...
            if (Iterations == Max_Iterations):
                ...
                print('当前种子号为：',Randomseed)
                return [0, 0]

            for trials in range(M):
                Solution_Feasibility[trials] = Judge_Feasibility(DecisionVarX[trials], DecisionVarY[trials], N_test,
                                                                 Utility_test, PairUtility_test)
            ...
            for TrialID in range(M):
                if (Solution_Feasibility[TrialID] == 1):
                    [Objective_Test[TrialID], Satisfaction_Test[TrialID], Profit_Test[TrialID]] = Evaluation(
                        DecisionVarX[TrialID], DecisionVarY[TrialID], DecisionVarZ[TrialID], DecisionVars[TrialID],
                        N_test,
        ...
        print('最优方案的gap、满意度、利润分别为：', Gap_Test[SelectedSol_ID], Satisfaction_Test[SelectedSol_ID],
              Profit_Test[SelectedSol_ID])
        ...
        return [Satisfaction_Test[SelectedSol_ID], Profit_Test[SelectedSol_ID],DecisionVarX[SelectedSol_ID]]
    ...
    def Deterministic_Output(N_test):
        ...
        [DecisionVarX_Deterministic, DecisionVarY_Deterministic, DecisionVarZ_Deterministic,
         DecisionVars_Deterministic, Objective1_Deterministic, Objective2_Deterministic,
         Flag_of_Infeasible] = Optimizer(1, 'Deterministic')
        ...
        Feasible_Flag = Judge_Feasibility(DecisionVarX_Deterministic, DecisionVarY_Deterministic, N_test,
                                          Utility_test, PairUtility_test)
        [Objective_Test, Satisfaction_Test, Profit_Test] = Evaluation(DecisionVarX_Deterministic,
                                                                      ...
                                                                      DecisionVars_Deterministic, N_test,
                                                                      Utility_test,
        ...
        return [Satisfaction_Test, Profit_Test, Feasible_Flag,DecisionVarX_Deterministic]
    ...
    Satisfaction_Result = []
    Profit_Result = []
    DecisionVarX_Stochastic_Result = []
    ...
    while (Epsilon < Epsilon_UB):
        ...
        [Satisfaction_Temp, Profit_Temp,DecisionVarX_Temp] = SAA(Initial_M, Initial_N, Test_SampleSize)
        DecisionVarX_Stochastic_Result.append(DecisionVarX_Temp)
    ...
    time_end=time.time()
    time_consumption=np.rint(time_end-time_start)
    print('Finished')
    print('共计用时：',time_consumption,'s')

    print('双目标优化结果如下：')
    for i in range(len(Satisfaction_Result)):
        print(Satisfaction_Result[i],Profit_Result[i])

    # print(pd.DataFrame([[1.0*Initial_M_Study,1.0*Initial_N_Study,1.0*Test_SampleSize_Study,1.0*k_Study,Initial_M,Initial_N,Test_SampleSize,k,Randomseed,Variance,time_consumption]],columns=['Initial_M_Study', 'Initial_N_Study', 'Test_SampleSize_Study','k_Study','Initial_M','Initial_N','Test_SampleSize','k','RandomSeed','Variance','time']))
    researchlabel=''
    researchvalue=0
    if Initial_M_Study==True:
        researchlabel='1000'
        researchvalue=Initial_M
    if Initial_N_Study==True:
        researchlabel='0100'
        researchvalue=Initial_N
    if Test_SampleSize_Study==True:
        researchlabel='0010'
        researchvalue=Test_SampleSize
    if k_Study==True:
        researchlabel='0001'
        researchvalue=k

    tmp=pd.DataFrame([[1.0*Initial_M_Study,1.0*Initial_N_Study,1.0*Test_SampleSize_Study,1.0*k_Study,Initial_M,Initial_N,Test_SampleSize,k,Randomseed,Variance,time_consumption]],columns=['Initial_M_Study', 'Initial_N_Study', 'Test_SampleSize_Study','k_Study','Initial_M','Initial_N','Test_SampleSize','k','RandomSeed','Variance','time'])

    # tmp.to_csv('研究标的%s 参数取值%f 种子%d.csv' % (researchlabel,researchvalue,Randomseed),header=True,index=False)
...
if __name__ == '__main__':
    ...
    Initial_M_List=[5]
    Initial_N_List=[20]
    Test_SampleSize_List=[1000]
    k_list=[0.0001]

    if Initial_M_Study==True:
        Initial_M_List=[i for i in arange(5,10,1)]
    if Initial_N_Study==True:
        Initial_N_List=[i for i in arange(20,61,10)]
    if Test_SampleSize_Study==True:
        Test_SampleSize_List=[i for i in arange(1000,1401,100)]
    if k_Study==True:
        k_list=[i for i in arange(0.0001,0.0006,0.0001)]



    for randomseed in RandomseedList:
        for variance in VarianceList:
            for initial_M in Initial_M_List:
                for initial_N in Initial_N_List:
                    for test_SampleSize in Test_SampleSize_List:
                        for kk in k_list:
                            # jincheng.append(multiprocessing.Process(target=running,args=(randomseed,variance,initial_M,initial_N,test_SampleSize,kk)))
                            FinalResults=FinalResults.append(running(randomseed,variance,initial_M,initial_N,test_SampleSize,kk),ignore_index=True)


    FinalResults= FinalResults[['Initial_M','time']]
    FinalResults= FinalResults.groupby('Initial_M')['time'].mean().reset_index()
    FinalResults.to_csv('Initial_M_Study.csv',header=True)

    plt.plot(FinalResults['Initial_M'].values,FinalResults['time'].values)
    plt.xlabel('Initial value of M')
    plt.ylabel('Computation time')
    # plt.title('Sensitivity analysis on the initial value of M')
    plt.savefig("Sensitivity analysis on the initial value of M.eps",format='eps')
