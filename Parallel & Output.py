...
RandomseedList = [66,108,200,201,202,203,204,205,206,207,208,209,210,250,270,280,290,300,306]
VarianceList = [0.6,0.8]



def running(Randomseed,Variance):
    ...
    length = len(Satisfaction_Result)

    shuchu=np.hstack((np.array(Satisfaction_Result).reshape(length,1),np.array(Profit_Result).reshape(length,1)))
    DecisionVarX_Stochastic_Result = np.array(DecisionVarX_Stochastic_Result).reshape((length, Num_Module))
    jieguo = pd.concat([pd.DataFrame(shuchu), pd.DataFrame(DecisionVarX_Stochastic_Result)], axis=1)
    pd.DataFrame(jieguo).to_csv('随机性%f 确定与否%d 约束%f 种子%d.csv' % (Width_Utility,IsStochastic,Variance,Randomseed),header=False,index=False,mode='a')
    ...
    DecisionVarX_Deterministic_Result=[]
    ...
    while (Epsilon < Epsilon_UB):
        print('Epsilon=', Epsilon)
        Past_Epsilon = Epsilon
        [Satisfaction_Temp, Profit_Temp, Feasible_Temp, DecisionVarX_Deterministic_Temp] = Deterministic_Output(Test_SampleSize)
        Feasible_for_Deterministic.append(Feasible_Temp)
        DecisionVarX_Deterministic_Result.append(DecisionVarX_Deterministic_Temp)
    ...
    shuchu = np.hstack((np.array(Satisfaction_Result).reshape(length, 1), np.array(Profit_Result).reshape(length, 1),np.array(Feasible_for_Deterministic).reshape(length, 1)))
    DecisionVarX_Deterministic_Result = np.array(DecisionVarX_Deterministic_Result).reshape((length, Num_Module))
    jieguo = pd.concat([pd.DataFrame(shuchu), pd.DataFrame(DecisionVarX_Deterministic_Result)], axis=1)
    pd.DataFrame(jieguo).to_csv('随机性%f 确定与否%d 约束%f 种子%d.csv' % (Width_Utility,IsStochastic,Variance,Randomseed),header=False,index=False,mode='a')
...
if __name__ == '__main__':
    ...
    for randomseed in RandomseedList:
        for variance in VarianceList:
            jincheng.append(multiprocessing.Process(target=running,args=(randomseed,variance)))
    for p in jincheng:
        p.start()
